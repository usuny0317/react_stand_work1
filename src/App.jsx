...
function App() {
  ...
  const removeUser = (id) => {
    ...
    const rest= users.filter((user)=>{return user.id!==id});
    setUsers([...rest])
  };
  ...
  return (
    <>
      ...
      <ul>
        ...
        {users.map((user)=>{
          return (
          <li key={user.id} style={{ display: "flex", justifycontent: "center", gap: "10px", border: "1px solid black"}}>
            <div style={{width:"100px" ,display:"flex", padding:"3px"}}>이름: {user.name}</div>
            <div style={{width:"100px" ,display:"flex",padding:"3px"}}>나이: {user.age}</div>
            <button style={{width:"200px",padding:"3px"}} onClick={()=>removeUser(user.id)}>삭제</button>
          </li>)
